...
You already know how to write a sequence diagram.

In the Modules unit, you used one to follow calls from `main.js` into `census.js`, show returned values, and represent repeated work with `loop` blocks.

Potion Shelf keeps that skill alive, but the system is getting a little bigger.

Now the participants include:

- the **browser**, which loads the application,
- `main.js`, which runs your JavaScript,
- `database.js`, which provides data through functions,
- and the **DOM**, which represents the live page.

That gives the sequence diagram a new job: help you track how browser loading, JavaScript execution, data requests, and DOM updates fit together in time.

You are not starting over with Mermaid. You are extending a diagramming skill you already have to a browser application.
...
It gives us a simple way to show the order of events instead of trying to keep the whole flow floating around in our heads at once. You already used that idea across JavaScript modules; now you will use it across the browser, JavaScript, data calls, and the DOM.
...
<HandbookChallenge
  title="Show the browser loading main.js"
  hints={[
    <>Keep all four participants from the starter diagram.</>,
    <>The first interaction belongs between <code>Browser</code> and <code>Main</code>.</>,
    <>Sequence diagrams read from top to bottom, so this load step should appear before any data or DOM work.</>,
  ]}
  expected={
    <pre className="m-0 whitespace-pre-wrap">{`Browser->>Main: load module script`}</pre>
  }
  solution={
  ...
  Browser->>Main: load module script`}</pre>
  }
>
  <p className="m-0">
    Open <code>/sequenceDiagram.mmd</code>. Keep the four starter participants and add one interaction showing the browser loading <code>main.js</code>. Preview the diagram and confirm that this is the first action in the flow and that the arrow label explains the step in plain language.
  </p>
</HandbookChallenge>
...
For now, the important thing is applying the diagram rules you already know to two new participants: the browser and the DOM.
...
We are reusing the tool in a new kind of program before we ask it to carry more weight.
