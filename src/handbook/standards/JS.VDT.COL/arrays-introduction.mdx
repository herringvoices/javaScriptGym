...
hints={[
"First item is songs[0].",
"Third item is songs[2].",
...
Every array knows how many items it has via the <code>.length</code> property. You read it with <em>dot notation</em>—just like <code>user.name</code> from the objects chapter—so “the length of <code>songs</code>” is <code>songs.length</code>.

Because indexes start at 0, the last item lives at index <code>array.length - 1</code> (for <code>songs</code>, that’s <code>songs.length - 1</code>).
