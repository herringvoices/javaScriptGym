...
<HandbookChallenge
  title="Inspect the keys and values of book1"
  hints={[
    <><code>Object.keys()</code> and <code>Object.values()</code> both need the object as their argument.</>,
    <>Store each returned array in its own variable so you can inspect the two shapes side by side.</>,
  ]}
  solution={
    <pre className="m-0 whitespace-pre-wrap">{`const book1Keys = Object.keys(book1);
...
>
  <p className="m-0">
    Create <code>book1Keys</code> from <code>Object.keys(book1)</code> and <code>book1Values</code> from <code>Object.values(book1)</code>. Log both arrays so you can compare the property labels with the data stored under them.
  </p>
</HandbookChallenge>
...
<HandbookChallenge
  title="Inspect book1 as key-value pairs"
  hints={[
    <><code>Object.entries()</code> returns one array whose items are two-value arrays.</>,
    <>The first slot of each pair is the property name and the second slot is its value.</>,
  ]}
  solution={
    <pre className="m-0 whitespace-pre-wrap">{`const book1Entries = Object.entries(book1);
console.log(book1Entries);

const firstEntry = book1Entries[0];
console.log(firstEntry[0]);
console.log(firstEntry[1]);`}</pre>
  }
>
  <p className="m-0">
    Create <code>book1Entries</code> with <code>Object.entries(book1)</code> and log the full array. Then store the first pair in <code>firstEntry</code> and log its key and value separately using indexes <code>0</code> and <code>1</code>.
  </p>
</HandbookChallenge>
