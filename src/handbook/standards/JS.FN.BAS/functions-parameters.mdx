...
<HandbookChallenge
  title="Give welcome a name parameter"
  hints={[
    <>A parameter is the name inside the function definition’s parentheses.</>,
    <>Use that parameter anywhere inside the function where the personalized value belongs.</>,
    <>Template literals can insert a parameter with <code>${'{'}name{'}'}</code>.</>,
  ]}
  solution={
...
>
  <p className="m-0">
    Refactor the existing <code>welcome</code> arrow function so it accepts one parameter named <code>name</code>. Use that parameter in the first log so the welcome message can change for different callers. Keep the unread-messages log unchanged.
  </p>
</HandbookChallenge>

You’ve now seen a function that can speak to different people just by changing the argument.
...
bookLine("The Hobbit", "J.R.R. Tolkien");
...
<HandbookChallenge
  title="Welcome someone by first and last name"
  hints={[
    <>Multiple parameters are separated by commas inside the function definition’s parentheses.</>,
    <>Each parameter can be used independently inside the function body.</>,
    <>A template literal can place both parameter values into the same string.</>,
  ]}
  solution={
...
>
  <p className="m-0">
    Change <code>welcome</code> so it accepts two parameters named <code>firstName</code> and <code>lastName</code>. Use both values in the welcome line, and keep the second log exactly as <code>"You have unread messages!"</code>.
  </p>
</HandbookChallenge>
