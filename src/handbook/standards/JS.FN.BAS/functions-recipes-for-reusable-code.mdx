...
<HandbookChallenge
  title="Create the welcome function"
  hints={[
    <>A function declaration begins with the <code>function</code> keyword followed by the function’s name.</>,
    <>The steps the function should remember belong inside its curly braces.</>,
    <>Declaring the function stores the recipe; it does not run the body yet.</>,
  ]}
  solution={
...
>
  <p className="m-0">
    Declare a named function called <code>welcome</code>. Its body should log exactly <code>"=== Welcome, User! ==="</code> and <code>"You have unread messages!"</code>, in that order. Do not call the function yet.
  </p>
</HandbookChallenge>
...
<HandbookChallenge
  title="Rewrite welcome as an arrow function"
  hints={[
    <>Keep the function’s behavior the same; only the declaration syntax changes.</>,
    <>An arrow function can be stored in a <code>const</code> variable.</>,
    <>The basic shape is <code>const name = () =&gt; {'{'} ... {'}'}</code>.</>,
  ]}
  solution={
...
>
  <p className="m-0">
    Rewrite <code>welcome</code> as a <code>const</code> arrow function while preserving the same two log statements in the same order.
  </p>
</HandbookChallenge>
