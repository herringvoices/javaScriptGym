...
"Use <code>&&</code> to combine the two checks in one condition.",
"Change <code>moisture</code> to <code>0.20</code> and confirm the message no longer logs.",
]}
...
if (seed.size >= 3 && seed.moisture < 0.15) {
console.log('Shuck the sunflower seeds.');
} // no log now; the moisture check fails`}</pre>
}

>

  <p className="m-0">
    <code>&amp;&amp;</code> is “both conditions must be true.” If either check fails, the whole combined condition is false.
  </p>
</HandbookChallenge>
...
"Test three cases:",
"• <code>{ name: 'sesame', size: 2 }</code>",
"• <code>{ name: 'poppy', size: 1 }</code>",
"• <code>{ name: 'poppy', size: 3 }</code>",
]}
answers={ <pre className="m-0 whitespace-pre-wrap">{`let seed = {
name: 'sesame',
size: 2,
};
...
seed = { name: 'poppy', size: 1 };
// size match → logs

if (seed.name === 'poppy' || seed.size <= 1) {
console.log('Press into oil.');
}

seed = { name: 'poppy', size: 3 };
// no match → nothing logs
...
JavaScript doesn’t do extra work if it doesn’t have to. When it sees `&&` or `||`, it evaluates from **left to right** and stops as soon as the answer is clear. That’s **short-circuiting**.
...
For now, think of short-circuiting as:

> “Skip extra checks when the first screen already decided the outcome.”

We’ll use that idea in more advanced patterns later.
...
"Then plug it into an AND:", <pre key="peek2" className="m-0 whitespace-pre-wrap">{`let seed = { size: 2, moisture: 0.1 };
...
console.log('Shuck');
    ...
    "Run it and watch which <code>Checking:</code> lines actually print.",
    "Then change <code>size</code> to <code>4</code> and run it again.",
  ]}
...
>
  ...
  <p className="m-0">
    Short-circuiting is JavaScript being lazy in a good way: it avoids extra checks when the first screen already decided the result.
  </p>
</HandbookChallenge>
...
"Test with:",
"• pumpkin, size 9, moisture 0.1 (roast)",
"• pumpkin, size 9, moisture 0.3 (don’t roast)",
"• sunflower, size 12, moisture 0.2 (roast, because huge)",
]}
answers={ <pre className="m-0 whitespace-pre-wrap">{`let seed = {
name: 'pumpkin',
size: 9,
...
seed = { name: 'pumpkin', size: 9, moisture: 0.1 };
// pumpkin AND dry → Roast
...
seed = { name: 'pumpkin', size: 9, moisture: 0.3 };
// pumpkin but too wet → no log
...
seed = { name: 'sunflower', size: 12, moisture: 0.2 };
// huge even though not pumpkin → Roast
...
>
  ...
  <p className="m-0">
    Parentheses are your way of saying, “These checks belong together; treat them as one combined screen before mixing in the next.”
  </p>
</HandbookChallenge>
...
<HandbookChallenge
title="Name your combined conditions"
hints={[
"Start with:", <pre key="peek1" className="m-0 whitespace-pre-wrap">{`const seed = {
  name: 'sunflower',
  size: 4,
...
"Try changing the seed to be tiny, or to be big but too wet, and see which cases trigger the message.",
]}
answers={ <pre className="m-0 whitespace-pre-wrap">{`let seed = {
name: 'sunflower',
size: 4,
...
// Make it tiny:
seed = { name: 'sunflower', size: 1, moisture: 0.2 };
const tinyIsBigAndDry = seed.size >= 3 && seed.moisture < 0.15;
const tinyIsTiny = seed.size <= 1;

if (tinyIsBigAndDry || tinyIsTiny) {
console.log('Special handling');
} // runs because it is tiny`}</pre>
}
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>Both conditions must be true for the whole thing to be true</td>
            <td><code>size &gt;= 3 &amp;&amp; moisture &lt; 0.15</code></td>
        </tr>
        <tr>
            <td>logical OR (<code>||</code>)</td>
            <td>At least one condition must be true for the whole thing</td>
            <td><code>name === 'sesame' || size &lt;= 1</code></td>
        </tr>
        <tr>
            <td>short-circuit</td>
            <td>Stopping early once the result is known</td>
            <td><code>false &amp;&amp; secondCheck</code> (second check never runs)</td>
        </tr>
        <tr>
            <td>combined condition</td>
            <td>A single condition built from smaller ones using <code>&amp;&amp;</code> or <code>||</code></td>
            <td><code>(isPumpkin &amp;&amp; isDry) || isHuge</code></td>
        </tr>
        <tr>
            <td>grouping</td>
            <td>Using parentheses to bundle checks together</td>
            <td><code>(A &amp;&amp; B) || C</code></td>
        </tr>
        <tr>
            <td>guard check</td>
            <td>A quick condition that prevents work when the rule fails</td>
            <td><code>if (!seed) return;</code></td>
        </tr>
        <tr>
            <td>threshold</td>
            <td>A comparison boundary used in a condition</td>
            <td><code>qualityScore &gt;= 90</code></td>
        </tr>
    </tbody>
</table>
